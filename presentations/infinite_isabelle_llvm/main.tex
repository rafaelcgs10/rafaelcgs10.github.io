...
\draw (396,42) node [anchor=north west][inner sep=0.75pt]   [align=left] {WM 4};
% Text Node
\draw (55,142) node [anchor=north west][inner sep=0.75pt]   [align=left] {\$\textbackslash lambda case e of\\DT t d => insert (t, d) buf\\WM wm =>\\ \ if \$\textbackslash exists\$ t \$\textbackslash in\$ (map fst buf)\textbackslash math-container\{\}. \ t \ \$\textbackslash le\$ wm\\ \ then\\ \ \ \ out <- filter (\$\textbackslash lambda\$ (t, \_) . t \$\textbackslash le\$ wm) buf\\ \ \ \ \ ts <- remdups (map fst out)\\ \ \ \ \ buf <- filter (\$\textbackslash lambda\$ (t, \_) . t > wm) buf\\ \ \ \ \ H <- H + (mset (map snd out))\\ \ \ \ \ push (map (\$\textbackslash lambda\$ t . DT t (H + (mset (map snd (filter (\$\textbackslash lambda\$ (t', \_) . t' \$\textbackslash le\$ t) out))))) ts) @ [WM wm]\\ \ else\\ \ \ \ \ push \ [WM wm]};
% Text Node
