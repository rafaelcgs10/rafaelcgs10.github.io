...
\end{frame}
  ...
  %         \foreach \x [count=\i] in {\is{0},\is{0},\is{0},\is{0}} {
  %           \node [on chain=0, tape] (n\i) {\x};
  %         }
  %         \node [right=.05cm of n4] {$\cdots$};
  %       \end{scope}
  %     \end{scope}
  %     \begin{scope}[shift={($(scope1.east)+(0.5cm,0)$)}]
  %       \node (eq) {$=$};
  %     \end{scope}
  %     \begin{scope}[shift={($(scope1.east)+(1.8cm,0)$)}]
  %       \tikzset{tape/.style={minimum size=.6cm, draw}}
  %       \begin{scope}[start chain=0 going right, node distance=0mm]
  %         \foreach \x [count=\i] in {\is{0},\is{0},\is{0},\is{0}} {
  %           \node [on chain=0, tape] (n\i) {\x};
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Example: counts elements satisfying \is{P}:
\begin{tcblisting}{hbox,listing only,listing options={language=isabelle,aboveskip=0pt,belowskip=0pt},size=fbox,boxrule=0pt,frame hidden,arc=0pt,colback=yellow!10!white}
...
\begin{figure}[!t]
    ...
    \caption{Difference between \is{incr_hist_op'} and \is{incr_hist_op}}
