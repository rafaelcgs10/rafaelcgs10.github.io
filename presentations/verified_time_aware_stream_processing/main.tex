...
\begin{frame}[fragile]
  ...
  \begin{figure}[!t]
    \centering
    \begin{tabular}{@{}l@{}}
      \text{\is{produce (batch_op [(t_0,**a**)]) stream_1 =}}
      \\
      \begin{tikzpicture}[scale=0.9, every node/.style={scale=0.9},background rectangle/.style={fill=yellow!10!white},show background rectangle]
        \tikzset{tape/.style={minimum size=.6cm, draw}}
        \begin{scope}[start chain=0 going right, node distance=0mm]
          \foreach \x [count=\i] in {\is{DT t_1 [(t_0,**a**),(t_0,**a**),(t_1,**c**)]},\is{WM t_1},\is{DT t_2 [(t_2,**b**)]},\is{WM t_2},\is{DT t_3 [(t_3,**a**)]}} {
            \ifnum\i=5
              \node [on chain=0, tape, outer sep=0pt] (n\i) {\x};
              \draw (n\i.north east) -- ++(.1,0) decorate [decoration={zigzag, segment length=.12cm, amplitude=.02cm}] {-- ($(n\i.south east)+(+.1,0)$)} -- (n\i.south east) -- cycle;
            \else
      ...
      \begin{tikzpicture}[scale=0.9, every node/.style={scale=0.9},background rectangle/.style={fill=yellow!10!white},show background rectangle]
        \tikzset{tape/.style={minimum size=.6cm, draw}}
        \begin{scope}[start chain=0 going right, node distance=0mm]
          \foreach \x [count=\i] in {\is{DT t_5 [(t_5,**a**),(t_5,**c**)]},\is{WM t_5}} {
            \ifnum\i=2
              \node [on chain=0, tape, outer sep=0pt] (n\i) {\x};
              \draw (n\i.north east) -- ++(.1,0) decorate [decoration={zigzag, segment length=.12cm, amplitude=.02cm}] {-- ($(n\i.south east)+(+.1,0)$)} -- (n\i.south east) -- cycle;
            \else
          ...
          \node [right=.25cm of n2] {$\cdots$};
        \end{scope}
      \end{tikzpicture}
    \end{tabular}
    \vspace*{-2ex}
    \label{fig:batch_output}
    \vspace*{-2ex}
  \end{figure}
\end{frame}
