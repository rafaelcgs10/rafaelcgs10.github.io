...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item $\EndOp \approx \SpinOp \approx \SilentOp$
    \item $\const{ex1} \approx \const{ex2} \approx \const{ex3}$
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Buffers $\var{buf} \oftype \tvar{a} \Fun \tvar{d}\ \type{list}$
  \end{itemize}
  \begin{tcolorbox}[enhanced,title=Buffer auxiliary functions,colback=yellow!30]
    \vspace*{-3ex}
    \hspace*{-5ex}
    \begin{align*}
...
\begin{frame}[fragile]
  ...
  \begin{tcolorbox}[enhanced,title=Identity operator,colback=yellow!30]
    ...
    \begin{align*}
      ...
      \\
      & \only<2-4>{(((\lambda \var{p}.\ \const{Read}\ \var{p}\ (\lambda \var{x}.\ \const{id_op}\ (\const{BENQ}\ \var{p}\ \var{x}\ \var{buf})))\ \Image_c \ \Univ_c)\ }
      \\
      &\Union_c
      \\
      &\only<3-4>{\phantom{(}((\lambda \var{p}.\ \const{Write}\ (\const{id_op}\ (\const{BTL}\ \var{p}\ \var{buf}))\ \var{p}\ (\const{BHD}\ \var{p}\ \var{buf}))\ \Image_c \ \{\var{p} \in_c \Univ_c \mid \var{buf}\ \var{p} \neq []\}))}
    \end{align*}
  \end{tcolorbox}
  \begin{itemize}
          \only<2-4>{\item $\Univ_c$ is the set of usable ports provide by its type}
    \item Stream delayer!
    \item $\IdOp = \const{id_op}\ (\lambda \_.\ [])$
  \end{itemize}
\end{frame}
